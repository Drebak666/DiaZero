...
// === helper: UID actual (prefiere sesión; si no, tabla usuarios por username) ===
...
export async function cargarIngredientesParaReceta() {
  ...
  // Usamos la tabla base y filtramos por UID
...
.eq('owner_id', uid);
  ...
  (data || []).forEach(ingrediente => {
    ...
    option.dataset.cantidadBase = ingrediente.cantidad || 0;
...
export async function guardarReceta() {
  ...
  try {
    const uid = await getUidActual();
    if (!uid) {
      showMessageModal('No hay sesión activa.');
      return;
    }
    ...
    // Guardar desglose en ingredientes_receta
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnAdd) {
    btnAdd.addEventListener('click', () => {
      ...
      const cantidadBase = parseFloat(option.dataset.cantidadBase) || 0;
